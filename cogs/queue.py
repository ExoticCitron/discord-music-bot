...
class Queue(commands.Cog):
    ...
    @app_commands.command(name="clear", description="Clear the queue while keeping the current song playing.")
    async def clear_command(self, interaction: discord.Interaction):
        ...
        if not music_cog:
            ...
            return
        if guild_id not in music_cog.song_queue or not music_cog.song_queue[guild_id]:
            empty_embed = discord.Embed(description="The queue is already empty.", color=0xFF0000)
            empty_embed.set_footer(text="https://divisionbot.space/")
            await interaction.response.send_message(embed=empty_embed)
            return
        queue_length = len(music_cog.song_queue[guild_id])
        music_cog.song_queue[guild_id] = []        
        success_embed = discord.Embed(
            description=f"Cleared **{queue_length}** songs from the queue. The current song will continue playing.",
            color=0x32CD32
        )
